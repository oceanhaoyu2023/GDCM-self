...
#         input_data_r = np.where(input_mask_r,data_norm,np.nan)
        ...
        return input_data, input_mask, target_data, target_mask,self.input_path_list[idx]
...
model3 = generator(7)
model3.to(device)
model3.load_state_dict(torch.load('./Model_Train_Results/model_last_save_revise_epoch_3_back_chlor_global.pth',map_location=device)['model_state_dict'])
...
for ep in range(1):
    ...
    for input_data_np, input_mask_np, target_data_np, target_mask_np, file_name in progress_bar:
#         print(file_name)
...
#                             for x_input_batch, mask_input_batch, x_target_batch, mask_target_batch in train_dataloader_2:
                            ...
                            outputs = model3(x_input, x_batch_decoder)
                            outputs = outputs.cpu().detach().numpy()                    
                            np.save('./DINEOF_MODEL_chlor_epoch3_global_mask/'+str(file_name)[48:-3],outputs)
