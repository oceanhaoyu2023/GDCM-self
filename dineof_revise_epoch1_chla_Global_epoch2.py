...
class SSTDatasetInit(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return input_data, input_mask, target_data, target_mask
...
model = generator(7)
model.to(device)
model.load_state_dict(torch.load('./Model_Train_Results/model_last_save_revise_epoch_2_back_chlor_global.pth',map_location=device)['model_state_dict'])

model1 = generator(7)
model1.to(device)
model1.load_state_dict(torch.load('./Model_Train_Results/model_last_save_revise_epoch_1_back_chlor_global.pth',map_location=device)['model_state_dict'])
...
train_dataloader_1 = DataLoader(train_dataset, batch_size=load_num, shuffle=True,drop_last=True)
...
for ep in range(30):
    ...
    for input_data_np, input_mask_np, target_data_np, target_mask_np in progress_bar:
    ...
    #         print(f"=== Iteration {iteration + 1} ===")
            ...
            train_dataset = SSTDatasetItem(input_data=input_data_np,
                                           ...
                                           max_modes=max_modes)
            train_dataloader_2 = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,drop_last=True)
            model.train()
    ...
    #             print(f"=== Epoch {epoch + 1} ===")
                total_loss = 0.
                total_samples = 0
    #             progress_bar = tqdm(train_dataloader_2, desc="Training")
    ...
    #                             for x_input_batch, mask_input_batch, x_target_batch, mask_target_batch in train_dataloader_2:
                                ...
                                outputs = model1(x_input, x_batch_decoder)
                                outputs = outputs.cpu().detach().numpy()

                                all_outputs.append(outputs)
                                all_targets.append(x_target_batch.numpy())
                                all_input_masks.append(mask_input_batch.numpy())
                                all_target_masks.append(mask_target_batch.numpy())

                    #             print("======= Finish Update =======")

                                outputs_data = np.concatenate(all_outputs, axis=0)
                                input_mask_np = np.concatenate(all_input_masks, axis=0)
                                target_mask_np = np.concatenate(all_target_masks, axis=0)
                                target_data_np = np.concatenate(all_targets, axis=0)
                        ...
                        x_batch_decoder = x_batch_decoder.to(device)
                        x_batch_decoder = torch.cat((background,x_batch_decoder),2)
                        mask_target_batch = mask_target_batch.to(device)

                        outputs = model(x_input, x_batch_decoder)
                        # 计算损失，只在已知数据位置计算
                        loss = criterion(outputs * mask_target_batch, x_target_batch.squeeze(2) * mask_target_batch)* 1000.

                        optimizer.zero_grad()
                        loss.backward()
                        optimizer.step()

                        total_loss += loss.item() * x_input_batch.size(0)   # 累加损失
                        total_samples += x_input_batch.size(0)

                        avg_loss = total_loss / total_samples
                        progress_bar.set_postfix({'Loss': loss.item() * x_input_batch.size(0)})

                        epoch_loss = loss.item()

                # 记录模型损失
    #             loss_history.append({'epoch':  epoch_num + 1, 'train_loss': epoch_loss})
                ...
                if epoch_num % 500 == 0:
                    ...
                    }, save_dir + f"model_last_save_revise_epoch_2_back_chlor_global.pth")

    #         print("======= Finish Training =======")

            # 更新 filled_data，用模型的输出替换缺失值位置
#             model.eval()
#             with torch.no_grad():
#                 all_outputs = []
#                 all_targets = []
...
plt.contourf( (background).cpu().detach().numpy()[0,0,0])
plt.colorbar()
...
plt.pcolormesh(x_input_batch.cpu().detach().numpy()[1,6,0]*20,cmap='jet',vmin=20,vmax=35)


# In[ ]:


plt.pcolormesh(outputs.cpu().detach().numpy()[0,6]*20,cmap='jet',vmin=20,vmax=30)
