...
model = generator(7)
model.to(device)
# model.load_state_dict(torch.load('./Model_Train_Results/model_last_save_revise_epoch_1_back_chlor.pth',map_location=device)['model_state_dict'])
...
for ep in range(30):
    ...
    #             progress_bar = tqdm(train_dataloader_2, desc="Training")
                for x_input_batch, mask_input_batch, x_target_batch, mask_target_batch in train_dataloader_2:
                    ...
                    # 计算损失，只在已知数据位置计算
                    loss = criterion(outputs * mask_target_batch, x_target_batch.squeeze(2) * mask_target_batch)

                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()

                    total_loss += loss.item() * x_input_batch.size(0) * 1000.  # 累加损失
                    total_samples += x_input_batch.size(0)

                    avg_loss = total_loss / total_samples
                    progress_bar.set_postfix({'Loss': loss.item() * x_input_batch.size(0) * 1000})

                epoch_loss = loss.item() * 1000.
    ...
    #             loss_history.append({'epoch':  epoch_num + 1, 'train_loss': epoch_loss})
                ...
                if epoch_num % 500 == 0:
                    ...
                    }, save_dir + f"model_last_save_revise_epoch_1_back_chlor_global.pth")
